...
# Use OpenRouter for access to more models
client = OpenAI(
...
Format: "Implied price: $X - [brief analysis of changes and prediction accuracy]" """
        ...
        # Make reasoning parameter optional for OpenRouter
        api_call_params = {
            "model": model,
            "messages": messages
        }
        ...
        ai_state['analysis'] = response.choices[0].message.content.strip()
        ai_state['model'] = model
        ai_state['effort'] = ""  # No effort parameter for OpenRouter
...
def main(stdscr, event_id, interval, hist):
    ...
    ai_state = {
        ...
        'previous_data': None,
        'updating': False,
        'conversation_history': []  # Keep last 8 interactions
    }
...
def cli():
    p = argparse.ArgumentParser(description="Retro terminal UI for Polymarket range markets (Yes/No pairs) - OpenRouter version.")
